function Out = mouth_warp_ec2(delta_w, delta_h1,delta_h2, fScale,image,file)

[im_height, im_width] = size(image);
open_mouth = rgb2gray(imread('open_mouth_adi.jpg'));
fileID = fopen(file,'r');
formatSpec = '%d';
neutral_w = 1;
neutral_h1 = 1;
...
for a=1:im_height
   for b=1:im_width
       if(a > 25 & a < 79 & b > 18 & b <115 )
            out_image(a,b) = open_mouth(a-5,b);
       end
   end
end
